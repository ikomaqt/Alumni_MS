...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !isset($_POST['action'])) {
    ...
    $skills = isset($_POST['skills']) && trim($_POST['skills']) !== '' ? $_POST['skills'] : 'None';
    $work_experience = isset($_POST['work_experience']) && trim($_POST['work_experience']) !== '' ? $_POST['work_experience'] : 'None';
            ...
            <form id="registration-form" method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" enctype="multipart/form-data">
                ...
                <div class="form-step" data-step="2">
                    ...
                    <div class="form-group">
                        ...
                        <select id="graduation_year" name="graduation_year" disabled>
                            <option value="">Select academic year</option>
                        </select>
                        <input type="hidden" id="hidden_graduation_year" name="graduation_year">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            async function validateStep(stepNumber) {
                ...
                // Step 3 validation
                    ...
                    // Remove skills validation to make it optional
            ...
            // Add Enter key navigation for step fields
            form.addEventListener('keydown', async function(e) {
                // Only handle Enter key
                if (e.key === 'Enter') {
                    // Find the active step
                    const activeStep = document.querySelector('.form-step.active');
                    // If focus is on a textarea, ignore Enter (allow newlines)
                    if (document.activeElement && document.activeElement.tagName === 'TEXTAREA') {
                        return;
                    }
                    // Prevent form submit on Enter
                    e.preventDefault();
                    // Validate current step, then go to next if valid
                    const validationResult = await validateStep(currentStep);
                    if (validationResult && currentStep < totalSteps) {
                        showStep(currentStep + 1);
                    }
            ...
            const hiddenGraduationYear = document.getElementById('hidden_graduation_year');
            let lastLrnValue = '';
            lrnInput.addEventListener('input', async function() {
                const lrn = lrnInput.value.trim();
                if (lrn === lastLrnValue) return;
                lastLrnValue = lrn;
                graduationYearSelect.disabled = true; // Always keep disabled
                ...
                if (/^\d{12}$/.test(lrn)) {
                    try {
                        ...
                        if (data.success && data.year) {
                            ...
                            hiddenGraduationYear.value = data.year;
                        } else {
                            graduationYearSelect.value = '';
                            hiddenGraduationYear.value = '';
                        }
                    } catch (e) {
                        graduationYearSelect.value = '';
                        hiddenGraduationYear.value = '';
